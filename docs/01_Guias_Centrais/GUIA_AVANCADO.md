...
- Guia Avançado v2.1 (documento anterior)
...
1. **Concepção e Refinamento de Ideias/Requisitos:**
    ...
    - `@AgenteMentorPO` auxilia na tradução para Histórias de Usuário (HUs) e Critérios de Aceite (ACs) a partir da [[02_Requisitos/ERS.md]] e insights do Maestro/`@AgenteOrquestrador`.
...
Conforme delineado no [[01_Guias_Centrais/PLANO_MESTRE_RECOLOCA_AI.md]] (v1.3, Seção 3.1) e expandido aqui:
...
- **Funcionalidades Detalhadas:**
    ...
    1. **Análise** da Documentação Estratégica e Tática (via RAG): Processa e "compreende" documentos chave do projeto (ex: [[01_Guias_Centrais/PLANO_MESTRE_RECOLOCA_AI.md]], [[02_Requisitos/ERS.md]], [[03_Arquitetura_e_Design/HLD.md]]) e, se disponível no RAG, materiais sobre Product Management (frameworks, artigos) para extrair contexto.
    ...
    3. **Criação Assistida de Prompts Eficazes:** Após o diálogo estratégico com o Maestro, auxilia na montagem de prompts detalhados e estruturados para os agentes de destino, incorporando o contexto estratégico validado, referências diretas à documentação, sugerindo técnicas de engenharia de prompt (Zero-Shot, Few-Shot, Chain-of-Thought) e utilizando os templates de [[05_Prompts/Templates_Base/]].
...
- `@AgenteMentorPO` **(Product Owner):**
    ...
    - **Tarefas:** Gerar HUs (em [[02_Requisitos/HU_AC/]]) e ACs a partir da [[02_Requisitos/ERS.md]] (v0.5), após alinhamento estratégico com o Maestro e o `@AgenteOrquestrador`. **Deve ser instruído a considerar o contexto estratégico fornecido pelo `@AgenteOrquestrador`**.
...
- **Base de Conhecimento:** Documentos Markdown do projeto (ERS, Plano Mestre, HLD, LLDs, este Guia, etc.) armazenados em [[08_Knowledge_Base_RAG_Sources/]].
    
    - **Base de Conhecimento Adicional para `@AgenteOrquestrador`:** Incluir artigos, resumos de livros e frameworks de Product Management em [[08_Knowledge_Base_RAG_Sources/PM_Knowledge/]].
...
O feedback fornecido pelo Maestro durante o HITL é crucial para o **refinamento contínuo** dos [[05_Prompts/Templates_Base/]], das regras em [[.trae/rules/]], e da base RAG. É um ciclo de aprendizado mútuo.
...
- **Estrutura e Clareza Externa:**
    ...
    - O **Kanban no Obsidian** ([[KANBAN_Recoloca_AI]]) oferece uma visualização clara do progresso e das prioridades, reduzindo a sobrecarga mental de ter que lembrar de todas as tarefas.
...
Ela está detalhada no [[01_Guias_Centrais/PLANO_MESTRE_RECOLOCA_AI.md]] e replicada aqui para referência rápida (pode haver pequenas variações conforme a evolução do projeto):
...
│   └── 09_Pesquisa_e_Insights/ # Movido para dentro de docs para organização
│       └── (Notas de pesquisa, artigos, etc.)
├── knowledge_base_rag/       # Fontes de dados para o sistema RAG
│   ├── ERS_para_RAG.md             # Versão da ERS otimizada para RAG
│   ├── PLANO_MESTRE_para_RAG.md    # Versão do Plano Mestre otimizada para RAG
│   ├── GUIA_AVANCADO_para_RAG.md   # Versão do Guia Avançado otimizada para RAG
│   ├── STYLE_GUIDE_para_RAG.md
│   ├── API_Specs_Sumario_para_RAG.md
│   └── PM_Knowledge/               # Materiais sobre Product Management
│       └── (Artigos e frameworks de PM)
├── logs/                       # Logs gerados por scripts ou processos (se aplicável localmente)
│   └── (arquivos de log)
├── scripts/                    # Scripts de automação do projeto
│   └── rag_indexer.py            # Script para indexar a base RAG
...
| -------------------------------------- | ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| **Estratégia e Orquestração**          | `@AgenteOrquestrador`              | **Atuar como PM Mentor:** Auxiliar o Maestro a validar estratégia de features, alinhar com objetivos, usar frameworks de PM. **Engenharia de Prompt:** Analisar docs (RAG), gerar perguntas, criar prompts eficazes. | Google Gemini (Pro via OpenRouter), Trae IDE, LangChain/FAISS (via RAG)                       | Toda a "Documentação Viva" (ERS, Plano Mestre, etc.), Materiais de PM em [[08_Knowledge_Base_RAG_Sources/PM_Knowledge/]]                        |
| **Definição de Requisitos**            | `@AgenteMentorPO`                  | Gerar Histórias de Usuário e Critérios de Aceite (em [[02_Requisitos/HU_AC/]]) a partir da [[02_Requisitos/ERS.md]] (v0.5), **considerando o alinhamento estratégico provido pelo `@AgenteOrquestrador`**.           | Google Gemini (Pro/Flash via OpenRouter), Trae IDE                                            | [[02_Requisitos/ERS.md]] (v0.5), [[01_Guias_Centrais/PLANO_MESTRE_RECOLOCA_AI.md]] (v1.3), Contexto estratégico do `@AgenteOrquestrador`        |
...
**Maestro:** "`@AgenteOrquestrador`, estou considerando o `RF-KAN-005` (Dashboard de Métricas Pessoais) da [[02_Requisitos/ERS.md]] v0.5 para a próxima sprint. Ajude-me a validar a estratégia e depois a preparar o prompt para o `@AgenteMentorPO` gerar as HUs."
