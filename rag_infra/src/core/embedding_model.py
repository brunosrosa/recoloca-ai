...
def initialize_embedding_model(force_cpu: bool = False, use_langchain: bool = True) -> Optional[EmbeddingModelManager]:
    """
    ...
    Returns:
        Optional[EmbeddingModelManager]: A instância do EmbeddingModelManager se o modelo foi inicializado com sucesso, None caso contrário.
    """
    global _embedding_manager
    ...
    if _embedding_manager is None:
        ...
        _embedding_manager = EmbeddingModelManager(force_cpu=force_cpu)
        if not _embedding_manager.load_model(use_langchain=use_langchain):
            logger.error("Falha ao carregar o modelo de embedding.")
            _embedding_manager = None
            return None
        logger.info("EmbeddingModelManager inicializado com sucesso.")
    else:
        logger.info("EmbeddingModelManager já inicializado.")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        if embedding is not None:
            ...
            print(f"[EMOJI] Info do cache: {manager.get_cache_info()}")
        else:
            print("[ERROR] Falha ao gerar embedding")
    else:
        print("[ERROR] Falha ao inicializar modelo")