...
try:
    ...
    print(f"[ERROR] Erro ao importar módulos RAG: {e}")
...
class RAGSetup:
    ...
    def check_gpu_availability(self) -> Tuple[bool, str]:
        ...
        self.logger.info("[SEARCH] Verificando disponibilidade de GPU...")
        
        if self.force_cpu:
            self.logger.info("[WARNING] Forçando uso de CPU")
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                self.logger.info(f"[OK] GPU detectada: {gpu_name} ({gpu_memory:.1f}GB)")
                return True, f"{gpu_name} ({gpu_memory:.1f}GB)"
            else:
                self.logger.warning("[WARNING] CUDA não disponível, usando CPU")
                return False, "CUDA não disponível"
        except ImportError:
            self.logger.warning("[WARNING] PyTorch não instalado, usando CPU")
    ...
    def create_directories(self) -> bool:
        ...
        self.logger.info("[EMOJI] Criando estrutura de diretórios...")
        ...
        try:
            for directory in directories:
                ...
                self.logger.debug(f"[OK] Diretório criado: {directory}")
                
            self.logger.info("[OK] Estrutura de diretórios criada")
            return True
    ...
    def check_source_documents(self) -> Tuple[bool, int]:
        ...
        self.logger.info("[EMOJI] Verificando documentos fonte...")
        ...
        for ext in supported_extensions:
            ...
            if files:
                self.logger.debug(f"[EMOJI] {len(files)} arquivos {ext} encontrados")
                
        if document_count == 0:
            self.logger.warning("[WARNING] Nenhum documento encontrado para indexar")
            self.logger.info(f"[EMOJI] Adicione documentos em: {SOURCE_DOCUMENTS_DIR}")
            return False, 0
        else:
            self.logger.info(f"[OK] {document_count} documentos encontrados")
    ...
    def test_embedding_model(self) -> bool:
        ...
        try:
            ...
            if embedding is not None and len(embedding) > 0:
                self.logger.info(f"[OK] Modelo carregado: {EMBEDDING_MODEL_NAME}")
                self.logger.info(f"[EMOJI] Dimensão do embedding: {len(embedding)}")
                return True
            else:
                self.logger.error("[ERROR] Falha ao gerar embedding")
                return False
                
        except Exception as e:
            self.logger.error(f"[ERROR] Erro ao testar modelo: {e}")
            return False
