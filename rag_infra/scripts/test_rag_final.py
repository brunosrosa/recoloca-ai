...
class RAGFinalTester:
    ...
    def _check_index_status(self) -> Dict[str, Any]:
        ...
        """
        try:
            from ..core_logic.constants import FAISS_INDEX_DIR, FAISS_DOCUMENTS_FILE, FAISS_METADATA_FILE
            
            status = {
                "index_dir_exists": FAISS_INDEX_DIR.exists(),
                "documents_file_exists": (FAISS_INDEX_DIR / FAISS_DOCUMENTS_FILE).exists(),
                "metadata_file_exists": (FAISS_INDEX_DIR / FAISS_METADATA_FILE).exists(),
                "documents_count": 0,
                "metadata_count": 0,
                "consistent": False
            }
    ...
    def _test_initialization(self) -> Dict[str, Any]:
        ...
        """
        logger.info("🔧 Testando inicialização do RAGRetriever...")
        ...
        try:
            ...
            configs_to_test = [
                ...
                {"force_pytorch": False, "use_optimizations": False}
            ]
            ...
            for i, config in enumerate(configs_to_test):
                try:
                    ...
                    if retriever.initialize():
                        ...
                        logger.info(f"✅ Inicialização bem-sucedida com configuração {i+1}")
                        break
                        
                except Exception as e:
                    logger.warning(f"⚠️ Configuração {i+1} falhou: {e}")
                    continue
            
            if not test_result["success"]:
                test_result["error"] = "Todas as configurações falharam"
                logger.error("❌ Falha na inicialização com todas as configurações")
        ...
        try:
            ...
            if retriever.initialize():
                ...
                if retriever.load_index():
                    ...
                    test_result["backend_type"] = type(retriever.backend).__name__ if hasattr(retriever, 'backend') else "Unknown"
                    
                    # Tentar obter tamanho do índice
                    if hasattr(retriever.backend, 'embeddings'):
                        test_result["index_size"] = len(retriever.backend.embeddings)
                    
                    logger.info(f"✅ Índice carregado: {test_result['index_size']} embeddings em {test_result['loading_time']:.3f}s")
                else:
                ...
                logger.error("❌ Falha na inicialização do retriever")
    ...
    def _test_searches(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for query in self.test_queries:
                try:
                    ...
                    if results:
                        ...
                        logger.info(f"✅ '{query}': {len(results)} resultados (score máx: {test_result['query_results'][query]['top_score']:.3f})")
                    else:
                        ...
                        logger.warning(f"⚠️ '{query}': Nenhum resultado")
            ...
            logger.info(f"📊 Resumo: {test_result['successful_queries']}/{test_result['total_queries']} queries bem-sucedidas")
    ...
    def _test_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            start_time = time.time()
            results = retriever.search(test_query, top_k=5, min_score=0.1)
            end_time = time.time()
            ...
            for query in batch_queries:
                retriever.search(query, top_k=3, min_score=0.1)
            ...
            logger.info(f"⚡ Performance: {test_result['single_search_time']:.3f}s por busca, {test_result['throughput_queries_per_second']:.1f} queries/s")
    ...
    def _test_different_thresholds(self) -> Dict[str, Any]:
        ...
        """
        logger.info("🎯 Testando diferentes thresholds...")
        ...
        try:
            ...
            for threshold in thresholds:
                try:
                    ...
                    logger.info(f"🎯 Threshold {threshold}: {len(results)} resultados (avg score: {result_info['avg_score']:.3f})")
                    
                except Exception as e:
                    test_result["threshold_results"][threshold] = {"error": str(e)}
                    logger.error(f"❌ Erro com threshold {threshold}: {e}")
            
            test_result["optimal_threshold"] = optimal_threshold
            logger.info(f"🎯 Threshold ótimo identificado: {optimal_threshold}")
            
        except Exception as e:
            logger.error(f"❌ Erro no teste de thresholds: {e}")
    ...
    def _generate_recommendations(self, report: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if summary.get("overall_success"):
                recommendations.append("✅ Sistema RAG funcionando corretamente")
                recommendations.append("🔧 Considere ajustar threshold para otimizar resultados")
                recommendations.append("📊 Monitore performance em produção")
            else:
                if not summary.get("system_functional"):
                    recommendations.append("🚨 CRÍTICO: Sistema não está funcional - verificar logs de erro")
                    recommendations.append("🔧 Verificar configuração do modelo de embedding")
                    recommendations.append("📁 Verificar integridade dos arquivos de índice")
                
                if summary.get("success_rate", 0) < 50:
                    recommendations.append("📈 Taxa de sucesso baixa - considere:")
                ...
                if not summary.get("performance_acceptable"):
                    recommendations.append("⚡ Performance lenta - considere:")
            ...
            if optimal_threshold != 0.1:
                recommendations.append(f"🎯 Considere usar threshold {optimal_threshold} para melhores resultados")
            
        except Exception as e:
            recommendations.append(f"❌ Erro na geração de recomendações: {e}")
    ...
    def _save_test_report(self, report: Dict[str, Any]):
        ...
        """
        try:
            report_path = Path("rag_final_test_report.json")
            with open(report_path, 'w', encoding='utf-8') as f:
                json.dump(report, f, indent=2, ensure_ascii=False, default=str)
            logger.info(f"📄 Relatório de testes salvo em: {report_path.absolute()}")
        except Exception as e:
    ...
    def _print_test_summary(self, report: Dict[str, Any]):
        ...
        if summary.get("overall_success"):
            ...
            print("⚠️ SISTEMA RAG PRECISA DE AJUSTES")
        
        print("📄 Relatório detalhado: rag_final_test_report.json")
        print("="*80)
...
def main():
    ...
    """
    print("🚀 Iniciando teste final do sistema RAG...")
